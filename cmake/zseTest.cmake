...
#
# Adds a googletest framework test to the ZSA test list
#
...
function(zsa_add_tests)
    cmake_parse_arguments(
        ZSA_ADD_TESTS                        # Prefix
        "HARDWARE_REQUIRED"                  # Options
        "TARGET;TEST_TYPE;WORKING_DIRECTORY" # One value keywords
        ""                                   # Multi value keywords
        ${ARGN})                             # args...

    if (NOT ZSA_ADD_TESTS_TARGET)
        message(FATAL_ERROR "No TARGET supplied to zsa_add_tests")
    endif()

    if (NOT ZSA_ADD_TESTS_TEST_TYPE)
        message(FATAL_ERROR "No TEST_TYPE supplied to zsa_add_tests")
    endif()

    if (NOT ZSA_ADD_TESTS_WORKING_DIRECTORY)
        set(ZSA_ADD_TESTS_WORKING_DIRECTORY "${CMAKE_RUNTIME_OUTPUT_DIRECTORY}/${CMAKE_CFG_INTDIR}")
    endif()

    get_property(TEST_TYPES GLOBAL PROPERTY TEST_TYPES)

    if (NOT ${ZSA_ADD_TESTS_TEST_TYPE} IN_LIST TEST_TYPES)
        string_join(" " TEST_TYPES_STR ${TEST_TYPES})
        message(
            FATAL_ERROR
            "Unknown TEST_TYPE ${ZSA_ADD_TESTS_TEST_TYPE} for target \
            ${ZSA_ADD_TESTS_TARGET}. Valid options are: ${TEST_TYPES_STR}")
    endif()

    string(TOLOWER ${ZSA_ADD_TESTS_TEST_TYPE} TEST_TYPE)
    set(HARDWARE_REQUIRED ${ZSA_ADD_TESTS_HARDWARE_REQUIRED})

    if ((${TEST_TYPE} STREQUAL "unit") AND (${HARDWARE_REQUIRED}))
        message(FATAL_ERROR "ZSA test (${ZSA_ADD_TESTS_TARGET}) may not
    ...
    if (NOT DEFINED ENV{ZSA_LOG_LEVEL})
        list(APPEND GTEST_PROPERTIES "ENVIRONMENT;ZSA_LOG_LEVEL=I;")
    endif()

    if (NOT DEFINED ENV{ZSA_ENABLE_LOG_TO_STDOUT})
        list(APPEND GTEST_PROPERTIES "ENVIRONMENT;ZSA_ENABLE_LOG_TO_STDOUT=1")
    endif()
    ...
    gtest_discover_tests(
        ${ZSA_ADD_TESTS_TARGET}
        EXTRA_ARGS
            "--gtest_output=xml:TEST-${ZSA_ADD_TESTS_TARGET}.xml"
        WORKING_DIRECTORY
            ${ZSA_ADD_TESTS_WORKING_DIRECTORY}
        ${TIMEOUT_VAR_NAME}
            60
        PROPERTIES
            "${GTEST_PROPERTIES}")
    ...
    if ("${CMAKE_CONFIGURATION_TYPES}" STREQUAL "")
        set_property(
            ...
            ${ZSA_ADD_TESTS_TARGET}${CMAKE_EXECUTABLE_SUFFIX})
